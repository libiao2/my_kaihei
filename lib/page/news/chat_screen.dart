...
class ChatScreen extends StatefulWidget {
  ...
  ChatScreen(
      {Key key, this.nnId, this.userName, this.avatar, this.isFriend})
      : super(key: key);
...
class _ChatScreenState extends State<ChatScreen> with TickerProviderStateMixin {
  List<ChatMessage> _messageWidgets = new List();
...
//  String inputValue;
  ...
  List<MessageItemEntity> _messages;

  //消息监听
  StreamSubscription<UpdateChatMessageEvent> _subscription;

  //页面监听 是否是好友引起页面变化
  StreamSubscription<AddFriendSuccessEvent> _screenUpdateSubscription;

  //发送消息 消息体
  MessageItemEntity _messageItem;
  ...
  double listHeight;
  double titleBarHeight = 51.0;
  double toolHeight = 100.0;
  ...
  void initState() {
    ...
    //当前页为聊天页面
    Constants.isOnChatScreen = true;
    Constants.curChatUserId = widget.nnId;
    ...
    //事件监听
    _subscription =
        Constants.eventBus.on<UpdateChatMessageEvent>().listen((event) {
      if (event.message != null) {
        //接收消息
        //页面更新
        createMessageItem(event.message);
        //添加到数据库
        DBHelper.insertMessage(event.message, widget.nnId);

        _messageItem = event.message;
      } else {
        //发送消息成功回调
        DBHelper.insertMessage(_messageItem, widget.nnId);
      }
    });

    _screenUpdateSubscription = Constants.eventBus.on<AddFriendSuccessEvent>().listen((event) {
      setState(() {
  ...
  void dispose() {
    ...
    Constants.isOnChatScreen = false;
    Constants.curChatUserId = null;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isFriend){
      ...
      listHeight = Constants.screenHeight -
          Constants.statusBarHeight -
          titleBarHeight -
          toolHeight - ScreenUtil().setHeight(56);
    }

    return Container(
      color: ColorUtil.lineGrey,
      child: Scaffold(
//          appBar: AppBar(
                  ...
                  Offstage(
                    ...
                    child: Container(
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          Text(Strings.addFriendToPlay, style: TextStyle(fontSize: ScreenUtil().setSp(14), color: ColorUtil.grey),),
                          InkWell(
                            onTap: (){
                              addFriend(widget.nnId);
                            },
                            child: Container(
                              alignment: Alignment.center,
                              height: ScreenUtil().setWidth(30),
                              width: ScreenUtil().setWidth(80),
                              child: Text(Strings.addFriend, style: TextStyle(fontSize: ScreenUtil().setSp(14), color: ColorUtil.white),),
                              decoration: BoxDecoration(
                            ...
                            if (curFilePath != null &&
                              ...
                              ImageUtil.uploadImg(file,
                                ...
                                if (res['code'] == 0) {
                                  ...
                                  _messageItem =
                                      MessageItemEntity(
                                          fromId: Constants.userInfo.nnId,
                                          toId: widget.nnId,
                                          content: url,
                                          timestamp: timestamp,
                                  ...
                                  ImageUtil.uploadImg(imageFile)
                                    ...
                                    if (response != null &&
                                      ...
                                      int timestamp =
                                          DateTime.now().millisecondsSinceEpoch;
                                      //发送图片 socket信息
                                      SocketHelper.sendMessage(widget.nnId, url,
...
//              onChanged: _onChanged,
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          Offstage(
            offstage: !widget.isFriend,
            child: iconBtn(
              imageUrl: "images/icon_phone.png",
              onPressed: () {
                if (FloatWindow.userNnId != null){
                  toast(Strings.calling);
                } else {
                  sendVoiceCall();
                }
              },
              ...
              PermissionHelper.checkPermission(
                ...
                ImageUtil.uploadImg(image).then((response) {
                  if (response != null && response["code"] == 0) {
                    ...
                    //发送图片 socket信息
                    SocketHelper.sendMessage(
                        widget.nnId, url, MessageType.IMAGE, timestamp);

                    _messageItem = MessageItemEntity(
                        fromId: Constants.userInfo.nnId,
                        toId: widget.nnId,
                        content: url,
                        timestamp: DateTime.now().millisecondsSinceEpoch,
  ...
  Future _handleSubmitted(String text) async {
    ...
    SocketHelper.sendMessage(widget.nnId, text, MessageType.TEXT, timestamp);
  ...
  ///获取聊天记录
  Future initUserMessage() async {
    if (Constants.connected) {
      //网络上获取数据
      var data = {"contact_id": widget.nnId, "last_id": 0, "limit": 100};
      request("post", allUrl["userMessage"], data).then((result) {
        if (result["code"] == 0 && result["data"]["list"] != null) {
          _messages = new List<MessageItemEntity>();
          List tempList = result["data"]["list"] as List;
          for (int i = tempList.length - 1; i >= 0; i--) {
            _messages.add(new MessageItemEntity.fromJson(tempList[i]));
          }

          //网络上获取数据就刷新数据库
          DBHelper.insertMessages(_messages, widget.nnId);
      ...
      //数据库读取数据
      _messages = await DBHelper.queryMessagesByUserId(widget.nnId);
    ...
    if (_messages != null && _messages.length > 0) {
      _messages.forEach((messageItem) {
        ChatMessage chatMessage = new ChatMessage(
          ...
          callback: sendVoiceCall,
        );
        _messageWidgets.add(chatMessage);
      });

      _messageItem = _messages[_messages.length - 1];
  ...
  void createMessageItem(MessageItemEntity itemEntity) {
    ChatMessage message = ChatMessage(
      messageItem: itemEntity,
      avatar: widget.avatar,
      callback: itemEntity.messageType == MessageType.VOICE_CALL
          ? sendVoiceCall
          : null,
...
//    SocketHelper.markMessageRead(widget.id);
  ...
  void sendVoiceCall() {
    PermissionHelper.checkPermission(
        context, "语音通话需要麦克风权限，是否前往打开应用权限？", PermissionGroup.microphone, () {
      SocketHelper.sendVoiceCall(widget.nnId, VoiceStatusType.SEND_REQUEST);

      AudioPlayerUtil.instance.voiceCallPlay();

      Navigator.push(context, MaterialPageRoute(builder: (context){
        return SendVoiceCallScreen(
            type: 1,
            nnId: widget.nnId,
...
//    return AspectRatio(
//        aspectRatio: assetList.height/assetList.width*Constants.keyBoardHeight,
//        child:Text(""));
